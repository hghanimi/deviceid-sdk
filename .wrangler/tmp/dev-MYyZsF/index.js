...
// src/server/services/Hasher.js
var require_Hasher = __commonJS({
  "src/server/services/Hasher.js"(exports, module) {
    init_modules_watch_stub();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    var crypto2 = require_crypto();
    var SignalHasher2 = class {
      static {
        __name(this, "SignalHasher");
      }
      hashSignals(signals) {
        const hashes = {
      ...
      _hash(input) {
        ...
        return crypto2.createHash("sha256").update(typeof input === "string" ? input : JSON.stringify(input)).digest("hex").substring(0, 32);
      }
      _hashCanvas(dataUrl) {
      ...
      _hashAudio(audioSum) {
        if (audioSum === null || audioSum === void 0) return null;
        return this._hash(Math.round(audioSum * 1e4) / 1e4);
      }
      _hashScreen(screen) {
    ...
    module.exports = new SignalHasher2();
  }
});

// src/server/matcher.js
var require_matcher = __commonJS({
  "src/server/matcher.js"(exports, module) {
    init_modules_watch_stub();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    var FuzzyMatcher2 = class _FuzzyMatcher {
      static {
      ...
      async findMatch(hashes, storedIds, apiKeyId) {
        ...
        if (storedMatch && storedMatch.confidence > 0.9) return storedMatch;
        ...
        if (bestScore >= _FuzzyMatcher.MATCH_THRESHOLD) {
          return {
    ...
    `, [
      ...
      _scoreMatch(incoming, candidate) {
        ...
        for (const { key, a, b } of comparisons) {
          const weight = _FuzzyMatcher.WEIGHTS[key];
          if (!a || !b) continue;
          totalWeight += weight;
    ...
    module.exports = FuzzyMatcher2;
  }
});

// src/server/services/graph.js
var require_graph = __commonJS({
  "src/server/services/graph.js"(exports, module) {
    init_modules_watch_stub();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    var crypto2 = require_crypto();
    var IdentityGraph2 = class {
      static {
        __name(this, "IdentityGraph");
      }
      constructor(db) {
        this.db = db;
      }
      ...
      async fireWebhooks(apiKeyId, eventData) {
        ...
        for (const hook of hooks.rows) {
          try {
            ...
            const signature = crypto2.createHmac("sha256", hook.secret).update(payload).digest("hex");
    ...
    module.exports = IdentityGraph2;
  }
});

// .wrangler/tmp/bundle-szQOxG/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-szQOxG/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
// src/client/index.js
var import_Hasher = __toESM(require_Hasher());
var import_matcher3 = __toESM(require_matcher());
...
app.use("*", async (c, next) => {
  ...
  const db = {
    query: /* @__PURE__ */ __name((text, params) => client.query(text, params), "query")
  };
  c.set("db", db);
  c.set("matcher", new import_matcher3.default(db));
  c.set("graph", new import_graph.default(db));
  await next();
...
async function rateLimit(c, next) {
  ...
  const now = Date.now();
  const windowMs = 6e4;
  const limit = apiKey.rate_limit || 1e3;
  ...
  if (entry.count > limit) {
    c.header("Retry-After", Math.ceil((entry.resetAt - now) / 1e3).toString());
    return c.json({
...
app.post("/v1/fingerprint", authenticate, rateLimit, async (c) => {
  ...
  try {
    ...
    const hashes = import_Hasher.default.hashSignals(signals);
    ...
    if (matchResult.isNew) {
      ...
      if (matchResult.matchType === "fuzzy" && matchResult.matchedSignals) {
        const crossBrowser = !matchResult.matchedSignals.includes("browser") && matchResult.matchedSignals.includes("canvas") && matchResult.matchedSignals.includes("webgl");
        if (crossBrowser && matchResult.originalVisitorId) {
          await graph.linkDevices(
            visitorId,
            matchResult.originalVisitorId,
            "cross_browser",
...
var middleware_insertion_facade_default = client_default;
